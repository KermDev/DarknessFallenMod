...
namespace DarknessFallenMod.NPCs.Bosses.PrinceSlime
{
    ...
    public class PrinceSlime : ModNPC
    {
        ...
        ref float MovingTimer => ref NPC.ai[0];
        void DoMoving()
        {
            Vector2 dirToPlayer = NPC.Center.DirectionTo(Target.Center);
            int xdir = MathF.Sign(dirToPlayer.X);
            ...
            if (NPC.velocity.Y == 0 && MovingTimer >= 90 && MortarTimer > 60)
            {
                MovingTimer = 0;
            ...
            if (NPC.velocity.Y == 0) MovingTimer++;
        }
        ...
        void DoShooting()
        {
            if (MortarTimer <= 0)
            {
                ...
                {
                    /*
                    Vector2 shootFrom = ShootPosition + dir * 45;

                    float angle = MathHelper.PiOver4;
                    float targDir = Target.Center.X - shootFrom.X;
                    float range = Math.Abs(targDir);

                    float speedPerS = MathF.Sqrt(range * PrinceSlimeFireballProjectile.GRAVITY * 60 / MathF.Sin(2 * angle));
                    */
                    ...
                    float speed60 = MathF.Sqrt(gravity * (shootAt.Y + MathF.Sqrt(MathF.Pow(shootAt.Y, 2) + MathF.Pow(shootAt.X, 2))));

                    float angle = MathF.Atan(
                        (MathF.Pow(speed60, 2) + MathF.Sqrt(MathF.Pow(speed60, 4) - (gravity * (gravity * MathF.Pow(shootAt.X, 2) + 2 * shootAt.Y * MathF.Pow(speed60, 2))))) / (gravity * shootAt.X)
                        );
                    ...
                    Vector2 initialVel = new Vector2(shootDir, 0).RotatedBy(angle * -shootDir) * speed60;
